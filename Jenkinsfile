		...
		stage('Test only powerapi') {
			...
			steps {
				script {
					...
					def pidPowerapi = sh (script: "(((powerapi duration 30 modules procfs-cpu-simple monitor --frequency 50 --console --pids ${output}) | grep muid) > data.csv) & echo $!",returnStdout: true)
